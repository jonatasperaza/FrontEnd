...
onMounted(async () => {
  orders.value = await orderStore.getOrderByClientOrByDriver(authStore.state.user.client.id, '') || []; 
});
...
const exampleOrder =     {
        ...
        "payment": {
            ...
            "pix_copyPaste": "00020126580014br.gov.bcb.pix013603fff190-7f9f-43ee-bb38-083eb807869a52040000530398654041.005802BR5925PERAZASUNAMITA202206062026009Sao Paulo62240520mpqrinter9579518328063049495",
            ...
            "ticket_url": "https://www.mercadopago.com.br/payments/95795183280/ticket?caller_id=1807436309&hash=06140a97-b30d-4e42-8132-de70eeb65b7f",
            "card": null,
            "installments": null
        },
        "address_delivery": {
  ...
  <div class="container">
    ...
    <div class="table">
      ...
      <div class="table-body">
        <div class="table-row" v-for="order in orders" :key="order.id" @click="router.push(`/order-status/${order.id}`)">
          <p>{{ order.id }}</p>
          <p>{{ order.items[0]?.name || 'N/A' }}</p>  
          <p>{{ order.items[0]?.weight || '0.00' }}</p>
          <p>{{ new Date(order.order_date).toLocaleDateString() }}</p>
          <p>{{ statusProcessor(order.status) }}</p>
          <p>{{ order.payment.status == 'approved' ? 'Pagamento aprovado' : order.payment.status == 'cancelled' ? 'Pagamento cancelado' : order.payment.status == 'pending' ? 'Pagamento pendente' : order.payment.status }}</p>
        </div>
      </div>
    </div>
  </div>
